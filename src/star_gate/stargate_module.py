...
class Block:
    ...
    def is_empty(self):
        # Check if Block contains key/value and/or not-empty Table
        if self.db == None:
            self._empty = True
        elif len(self.db.values()) == 1:
            item = self.db.values()[0]
            if isinstance(item,Table):
                self._empty = item.is_empty()
            else:
                self._empty = False
        else:
            self._empty = False
    ...
    def add(self,tdata,tname = 'table'):
        ...
        if isinstance(tdata,Table):
            self.db[tname] = tdata
            self._empty = False if tdata.is_empty() else True
        else:
            t = Table(tname)
            t.from_data(tdata)
            self.db[tname] = t
    ...
    def to_star(self,name="StarGate"):
        regex = r'[ \t\'\/]'
        s = f"data_{self.db_id}\n#\n# File generated by {name} on {datetime.now().strftime('%Y-%m-%d at %H:%M')}\n#\n"
...
class StarGate:
    ...
    def save(self,filename,orient='split',name="StarGate"):
        """
